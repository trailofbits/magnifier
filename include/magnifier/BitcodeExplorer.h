...
private:
    ...
    // ID of the `!explorer.id` metadata. This metadata holds the unique ID of this value.
    const unsigned md_explorer_id;
    // ID of the `!explorer.source_id` metadata. This metadata tracks the provenance of
    // a value. If a value, e.g. an instruction, is from a function, then usually the `!explorer.id`
    // and `!explorer.source_id` match. However, if the value has been subject to mutation or
    // inlining, then the `!explorer.source_id` will stay constant while the `!explorer.id` will
    // change.
    const unsigned md_explorer_source_id;
    // ID of the `!explorer.block_id` metadata. This metadata should only be attached to
    ...
    // This is a vector of all the llvm `Module` objects ingested using `TakeModule`.
    ...
    // This map helps keep track of hooks that needs to be elided during an operation.
    // It should be cleared at the end of any high-level operation.
    std::map<llvm::Type *, llvm::FunctionCallee> hook_functions;

    // Update/index a function by adding various metadata to function, instruction, and block values.
    // Also update `function_map`, `instruction_map`, and `block_map` to reflect the changes.
...
public:
    ...
    // It updates `opened_modules` and indexes all the functions inside the module.
    void TakeModule(std::unique_ptr<llvm::Module> module);

    // Invoke `callback` on every indexed function while providing its `ValueID` and `FunctionKind`.
    void ForEachFunction(const std::function<void(ValueId, llvm::Function & , FunctionKind)> &callback);
    ...
    [[nodiscard]] ValueId GetId(const llvm::Instruction &instruction, ValueIdKind kind) const;
    ...
    void SetId(llvm::Instruction &instruction, ValueId value, ValueIdKind kind) const;
